...
def obtener_productos(id_categoria):
    return conn.query(f"SELECT id, nombre FROM productos WHERE categoria_id = {id_categoria} ORDER BY nombre ASC;")
...
if not df_prod.empty:
    ...
    query_historial = f"""
        SELECT h.supermercado, h.precio, h.fecha 
        FROM historial_precios h
        WHERE h.producto_id = {id_prod_sel}
        ORDER BY h.fecha ASC;
    """
    df_historial = conn.query(query_historial)
    df_historial["fecha"] = pd.to_datetime(df_historial["fecha"])
    
    # --- INTERFAZ PRINCIPAL ---
    # Métricas del día más reciente cargado
